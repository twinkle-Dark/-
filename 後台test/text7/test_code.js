...
function doGet() {
  ...
  console.log (urlF)
  var template=HtmlService.createTemplateFromFile("管理後台");
...
function getDataVInfo(){
  ...
  var ss=SpreadsheetApp.openById(SPREADSHEET_ID);
  var museumSheet = ss.getSheetByName("VisitorInfo");
  var data=museumSheet.getDataRange();
  return data.getValues();

}
// 抓試算表裡VisitorInteractions的資料
function getDataVInteractions(){

  var ss=SpreadsheetApp.openById(SPREADSHEET_ID);
  var museumSheet = ss.getSheetByName("VisitorInteractions");
  var data=museumSheet.getDataRange();
  return data.getValues();
...
// 在Code.gs中檢查include函數定義
function include(filename) {
  try {
    return HtmlService.createHtmlOutputFromFile(filename).getContent();
  } catch(e) {
    console.error('無法載入檔案:', filename, e);
    return '<!-- 載入失敗:' + filename + ' -->';
  }
}
/**
